...
* 배열의 이름은 배열의 첫번째 요소의 포인터로 형변환 된다
    * 즉, 포인터처럼 사용할 수 있단 것 
* a[b] == *(a+b)
* 이중배열은 배열의 포인터를 담는 배열과 같다
    * int arr[2][3]에서 arr[1] 은 포인터가 담겨있다
    * int를 담는 길이 3의 배열이 2개가 있고, 각각의 배열의 주소가 int * 를 담는 길이 2의 배열에 담겨있다 생각하면 됨
    //????????
...
* return type 를 지정해주는 이유
    * 컴파일러가 함수의 반환값을 알 수 있게 하기 위해서
    * 실제로 코딩할 때도 linter를 통해 반환형을 활용 할 수 있다
